...
public class qaBank {
	...
	public int[] incOrRand(boolean incOrRand) {//incOrRand=true: Increasing difficulty. False: Random Questions
		int[] arr = {0,1,6,7,12,13,2,3,8,9,14,15,4,5,10,11,16,17}; //Question index array for inc difficulty
		if (incOrRand) {
			return arr;
		} else {
			Random rand = new Random();
			for (int i = 0; i < arr.length; i++) { //Randomly shuffle array
	...
	public String returnQuestion(int q) {
		String[] questionArr = {
				...
				"True/False: If p is a lower bound of Y in (S,partial order),\n and there is an element q in the lower bounds of Y such "
				+ "that p partial order q holds for all p in the lower bounds of Y,\n then q is the minimum of Y in (S, partial order)",
