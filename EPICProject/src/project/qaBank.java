...
public class qaBank {
	public int[] incOrRand(boolean incOrRand) {//incOrRand=true: Increasing difficulty. False: Random Questions
		int[] arr = {0,1,6,7,12,13,2,3,8,9,14,15,4,5,10,11,16,17}; //Array of question indexes for inc difficuly
		if (incOrRand) {
			return arr;
		} else if (!incOrRand) { 						// Array is randomly shuffled
	...
	public String returnQuestion(int q) {
		String[] questionArr = {
				...
				"True/False: If p is a lower bound of Y in (S,partial order), and there is an element q in" 
				+ "the lower bounds of Y such that p partial order q holds for all p in the lower bounds of X, then q is the minimum of X in (S, partial order)",
	...
	public String returnAnswer(int i, int j) {
		String[][] choiceAnswer = { //Correct answers in comments
		// Discrete Mathematics: q=0,1,2,3,4,5 | Novice:0,1 | Intermediate: 2,3 | Advanced: 4,5
		{"Or","Exclusive Or","If","If and only if"}, //3
		{"n^2","2^n","2n","n^n"}, //1
		{"p^p'","(p → q) ↔ (q ∨ p')","(p → q) ^ (q → p)","p⊕p"},//1
		{"3","7","8","6"}, //2
		{"Alternating zeros and ones","Square Matrix with a diagonal of ones, from top left to bottom right, and zero otherwise",
		"A square matrix of ones","A rectangular matrix of side ones and zero otherwise"},//1
		...
		{"The PSU not supplying sufficient voltage","Data loss in hard drives","The CPU awaiting instruction from cache, memory or hard drives",
		"Newer operating systems running on older components"},//2
	...
	public int correctAnswer(int k) {// k+1 = question number, returns corresponding correct j value for answerChoice method
		int[] correctAnswer = {3,1,1,2,1,1,2,1,0,3,1,0,0,2,1,3,1,2};
